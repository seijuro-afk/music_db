...
class MusicPlayer:
    def __init__(self, root, acc_id):
        ...
        self.account_button = tk.Button(self.main_frame, text="Playlist Controls", bg="#2e2e2e", fg="white", relief=tk.FLAT, command=lambda: open_playlist_control_gui(self.acc_id))
    ...
    def fetch_playlists_for_account(self):
        ...
        if connection is not None:
            cursor = connection.cursor()
            query = f"SELECT p.name FROM playlist p JOIN account a ON p.account_id = a.account_id WHERE a.account_id = {self.acc_id[0]}"
