...
def open_playlist_control_gui(account_id):
    ...
    playlist_window = Tk()
    playlist_window.title(f"Playlist Control - Account ID: {account_id[0]}")
    ...
    def fetch_playlists():
        """Fetch all playlists for the specified account_id."""
        connection = connect_to_db()
        if connection:
            try:
                cursor = connection.cursor()
                cursor.execute("SELECT playlist_id, name, created_at FROM playlist WHERE account_id = %s", (account_id))
        ...
        if connection:
            try:
                ...
                cursor.execute(
                    ...
                    (playlist_name, created_at, account_id[0])
                )
                connection.commit()
    ...
    def delete_playlist():
        ...
        if connection:
            try:
                cursor = connection.cursor()
                # Delete from playlistssongs first (foreign key constraint)
    ...
    # Title Label
    title_label = Label(
        playlist_window, text=f"Playlist Control for Account ID: {account_id[0]}", 
